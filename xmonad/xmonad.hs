...
myRofiCmd="rofi -matching fuzzy -show-icons -markup -tokenize -show combi"
myRofiMojiCmd="rofimoji -s neutral -a copy"
myDatabrowser = "caja"
...
myWorkspaces    = ["web","terminal","editor","msg","5","6","7","8","9"]
...
myKeys conf@(XConfig {XMonad.modMask = modm}) = M.fromList $
    ...
    -- launch Rofi
    , ((modm,               xK_d     ), spawn myRofiCmd)
    , ((modm,               xK_a     ), spawn myRofiMojiCmd)
...
--
main = do
    xmproc <- spawnPipe "xmobar -x 1 $HOME/.config/xmobar/xmobar-2.config"
    xmproc <- spawnPipe "xmobar -x 0 $HOME/.config/xmobar/xmobar.config"
    xmonad $ docks $ ewmh $ defaults xmproc
defaults xmproc= def {
      -- simple stuff
        terminal           = myTerminal,
